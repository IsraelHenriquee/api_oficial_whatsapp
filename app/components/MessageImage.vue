<template>
  <div>
    ...
    <form @submit.prevent="sendImage" class="space-y-6">
      ...
      <div>
        ...
        <input
          v-model="mediaId"
          type="text"
          placeholder="Ex: 1479537139650973"
          class="w-full px-3 py-2 bg-background border border-input rounded-md focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent"
        />
        <p class="text-xs text-muted-foreground mt-1">
      ...
      <!-- Campo URL da Imagem (Alternativa) -->
      <div>
        <label class="block text-sm font-medium text-foreground mb-2">
        ...
        />
        <p class="text-xs text-muted-foreground mt-1">
          URL pública da imagem (use Media ID ou URL, não ambos)
        </p>
      </div>
      ...
      <div>
        ...
        ></textarea>
        <p class="text-xs text-muted-foreground mt-1">
          Texto que acompanha a imagem (opcional)
        </p>
      </div>
    ...
    <div v-if="success" class="mt-6 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
      <h4 class="text-sm font-medium text-green-800 dark:text-green-200 mb-2">✅ Imagem enviada com sucesso!</h4>
...
const sendImage = async () => {
  ...
  try {
    ...
    error.value = err.data?.message || 'Erro ao enviar imagem'
  } finally {
